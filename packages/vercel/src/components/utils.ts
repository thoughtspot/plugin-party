...
export const getEnvVariables = async () => {
    ...
    const userData = await vercelPromise(`https://api.vercel.com/v2/user`, accessToken)
    const projectData = await vercelPromise(`https://api.vercel.com/v9/projects?teamId=${teamId}`, accessToken)
    ...
    if(projectData?.projects.length){
      projectData.projects.map( async (project) => {
        envPromises.push(
          vercelPromise(`https://api.vercel.com/v8/projects/${project.id}/env?teamId=${teamId}&decrypt=true&source=vercel-cli:pull`, accessToken)
        );
        envPromises.push(
          vercelPromise(`https://api.vercel.com/v8/projects/${project.id}/domains?teamId=${teamId}`, accessToken)
        );
      })
    }
    const projectsResponses = await Promise.all(envPromises)
    let connectionConfig, authUrl, hostUrl, projectIds : any[] = [];
    ...
    for(let index = 0; index<projectsResponses.length; index+=2){
      ...
      if(Object.keys(connectionParams).length === 5){
        ...
        authUrl = projectsResponses[index+1].domains[0].name;
  ...
  try {
    ...
    if (response.ok) {
      ...
      projectIds.forEach((projectId) => {
        fetch(`https://api.vercel.com/v10/projects/${projectId}/env?upsert=true&teamId=${teamId}`, {
          body: JSON.stringify([{
            key: 'TS_SECRET_KEY',
            value: secretKey,
            type: 'encrypted',
